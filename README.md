...
Behat não foi instalado localmente devido a diferenças de versão do PHP; recomendo rodar Behat em um container Docker com PHP >= 8.1. Exemplo de comando (documentado abaixo) instala um container e executa Behat dentro dele sem tocar seu PHP local.

Exemplo (documentado):

- `docker-compose-behat.yml` (exemplo no README) descreve um serviço `behat` usando `php:8.1` e composer para instalar dependências de BDD dentro do container.
