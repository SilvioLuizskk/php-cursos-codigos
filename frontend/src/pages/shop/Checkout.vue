<template>
    <div class="container mx-auto px-4 py-8">
        ...
        <div v-if="loading" class="flex justify-center py-12">
            <LoadingSpinner text="Carregando carrinho..." />
        </div>
            ...
            <div class="lg:col-span-2 space-y-6">
                ...
                <div class="bg-white p-6 rounded-lg shadow">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        <Input
                            v-model="formData.shipping_address.street"
                            label="Rua *"
                        ...
                        <Input
                            v-model="formData.shipping_address.number"
                            label="Número *"
                            placeholder="123"
                        ...
                        <Input
                            v-model="formData.shipping_address.complement"
                            label="Complemento"
                        ...
                        <Input
                            v-model="formData.shipping_address.state"
                            label="Estado *"
                            placeholder="UF"
                        ...
                        <Input
                            v-model="formData.shipping_address.zip_code"
                            label="CEP *"
...
const isFormValid = computed(() => {
    return (
        ...
        formData.shipping_address.number &&
...
async function applyCoupon() {
    ...
    try {
        ...
        showError(err.response?.data?.message || "Erro ao aplicar cupom");
    } finally {
...
function validateForm() {
    errors["shipping_address.street"] = !formData.shipping_address.street
        ? "Rua é obrigatória"
        : "";
    errors["shipping_address.number"] = !formData.shipping_address.number
        ? "Número é obrigatório"
        : "";
    errors["shipping_address.city"] = !formData.shipping_address.city
        ? "Cidade é obrigatória"
        : "";
    errors["shipping_address.state"] = !formData.shipping_address.state
        ? "Estado é obrigatório"
        : "";
    errors["shipping_address.zip_code"] = !formData.shipping_address.zip_code
        ? "CEP é obrigatório"
        : "";
    ...
    nextTick(() => {
        ...
        if (firstError) {
            const el = document.querySelector(
                `[name='${firstError.replace("shipping_address.", "")}']`
            );
            if (el) el.focus();
        }
    });
...
async function submitOrder() {
    ...
    try {
        const orderData = {
            ...
            billing_address: formData.shipping_address, // Mesmo endereço por padrão
